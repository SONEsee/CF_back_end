...
func CreateCustomerController(c echo.Context) error {
	var req requestbody.CustomerRequestBody
	...
	if err := services.CreateCustomerServices(c.Request().Context(), req); err != nil {
		log.Printf("create customer error: %v", err)
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ບໍ່ສາມາດສ້າງລູກຄ້າໄດ້", err.Error()))
	}
	return c.JSON(http.StatusOK, presenters.ResponseSuccess("SUCCESS"))
}

func GetDataCustomerController(c echo.Context) error {
	...
	items, paginationResult, err := services.GetDataCustomerServices(ctx, id, page, pageSize)
	if err != nil {
...
func UpdateCustomerPatchController(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ID ບໍ່ຖືກຕ້ອງ", "ກະລຸນາລະບຸ ID ເປັນຕົວເລກ"))
	}
	var req requestbody.CustomerPatchRequest
	if err := validators.ParseAndValidateBody(c, &req); err != nil {
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ຂໍ້ມູນບໍ່ຖືກຕ້ອງ", err.Error()))
	}
	if err := services.UpdateCustomerServicesPatch(c.Request().Context(), id, req); err != nil {
		if strings.Contains(err.Error(), "not found") {
			return c.JSON(http.StatusNotFound, presenters.ResponseError("ບໍ່ພົບຂໍ້ມູນ", err.Error()))
		}
		log.Printf("patch customer error: %v", err)
...
func DeleteCustomerController(c echo.Context) error {
	...
	if err := services.DeleteCustomerServices(c.Request().Context(), id); err != nil {
		if strings.Contains(strings.ToLower(err.Error()), "not found") {
			return c.JSON(http.StatusNotFound, presenters.ResponseError("ບໍ່ພົບຂໍ້ມູນ", err.Error()))
		}
		if strings.Contains(err.Error(), "violates foreign key constraint") {
			return c.JSON(http.StatusConflict, presenters.ResponseError("ລົບບໍ່ໄດ້", "ລູກຄ້າຄົນນີ້ຍັງມີປະຫວັດການສັ່ງຊື້ຢູ່"))
		}
