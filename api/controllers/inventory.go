...
func GetDataInventoryController(c echo.Context) error {
	...
	items, paginationResult, err := services.GetDataInventoryServices(ctx, id, page, pageSize)
	if err != nil {
...
// UpdateInventoryPatchController ອະນຸຍາດແກ້ໄດ້ສະເພາະ reorder_level — qty ຕ້ອງປ່ຽນຜ່ານ stock_movements ເທົ່ານັ້ນ
func UpdateInventoryPatchController(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ID ບໍ່ຖືກຕ້ອງ", "ກະລຸນາລະບຸ ID ເປັນຕົວເລກ"))
	}
	var req requestbody.InventoryPatchRequest
	...
	if err := services.UpdateInventoryServicesPatch(c.Request().Context(), id, req); err != nil {
		if strings.Contains(err.Error(), "not found") {
			return c.JSON(http.StatusNotFound, presenters.ResponseError("ບໍ່ພົບຂໍ້ມູນ", err.Error()))
		}
		log.Printf("patch inventory error: %v", err)
