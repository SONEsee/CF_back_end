...
func CreateProductController(c echo.Context) error {
	var req requestbody.ProductRequestBody
	...
	if err := services.CreateProductServices(c.Request().Context(), req); err != nil {
		log.Printf("create product error: %v", err)
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ບໍ່ສາມາດສ້າງສິນຄ້າໄດ້", err.Error()))
	}
	return c.JSON(http.StatusOK, presenters.ResponseSuccess("SUCCESS"))
}

func GetDataProductController(c echo.Context) error {
	...
	items, paginationResult, err := services.GetDataProductServices(ctx, id, page, pageSize)
	if err != nil {
...
func UpdateProductPatchController(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ID ບໍ່ຖືກຕ້ອງ", "ກະລຸນາລະບຸ ID ເປັນຕົວເລກ"))
	}
	var req requestbody.ProductPatchRequest
	if err := validators.ParseAndValidateBody(c, &req); err != nil {
		return c.JSON(http.StatusBadRequest, presenters.ResponseError("ຂໍ້ມູນບໍ່ຖືກຕ້ອງ", err.Error()))
	}
	if err := services.UpdateProductServicesPatch(c.Request().Context(), id, req); err != nil {
		if strings.Contains(err.Error(), "not found") {
			return c.JSON(http.StatusNotFound, presenters.ResponseError("ບໍ່ພົບຂໍ້ມູນ", err.Error()))
		}
		log.Printf("patch product error: %v", err)
...
func DeleteProductController(c echo.Context) error {
	...
	if err := services.DeleteProductServices(c.Request().Context(), id); err != nil {
